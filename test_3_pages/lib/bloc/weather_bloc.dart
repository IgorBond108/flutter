...
//на событие SearchCity вызывается метод _onSearchCity
  ...
  void _onSearchCity(WeatherFetchEvent event, Emitter<WeatherState> emitter) async {
...
//если какие-то события пришли
    if (event is SearchCity) {
      ...
      forecastObject = await WeatherApi()
          .fetchWeatherForecast(city: event.cityQuery) // который принимает название города
          .then((forecastObject) {
